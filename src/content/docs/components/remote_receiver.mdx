...
- **dump** (*Optional*, list): Decode and dump these remote codes in the logs (at log.level=DEBUG).
  ...
  - **onkyori**: Decode and dump Onkyo RI codes. See
    [transmitter description](/components/remote_transmitter#remote_transmitter-transmit_onkyori) for wiring and voltage-level details.
...
- **on_onkyori** (*Optional*, [Automation](/automations)): An automation to perform when an
  Onkyo RI code has been decoded. A variable `x` of type <APIStruct text="remote_base::OnkyoRIData" path="remote_base::OnkyoRIData" />
  is passed to the automation for use in lambdas.
...
- **onkyori**: Trigger on a decoded Onkyo RI code with the given data.

  - **data** (**Required**, int): The Onkyo RI code to trigger on, see dumper output for more info.
